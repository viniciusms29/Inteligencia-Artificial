...
def crossover(pais):
	...
	# 		
	for j in range((len(pai1.VetItens) -1), -1, -1):
		# Filho 3:
		if (pai1.VetItens[j] not in filho3):
			filho3.append(pai1.VetItens[j])
		if (pai2.VetItens[j] not in filho3):
			filho3.append(pai2.VetItens[j])

		# Filho 4:
		if (pai2.VetItens[j] not in filho4):
			filho4.append(pai2.VetItens[j])
		if (pai1.VetItens[j] not in filho4):
			filho4.append(pai1.VetItens[j])
...
def removerIndividuos(populacao):
	""" Remove os quatro individuos menos aptos. """
	populacao.pop()
	populacao.pop()
	populacao.pop()
	populacao.pop()
...
if __name__ == '__main__':
	...
	for i in range(0, noGeracoes):
		...
		individuoF3 = Individuo(entrada.ListaItens, entrada.Capacidade, filhos[2], 3)
		individuoF4 = Individuo(entrada.ListaItens, entrada.Capacidade, filhos[3], 3)
		populacao.append(individuoF1)
		populacao.append(individuoF2)
		populacao.append(individuoF3)
		populacao.append(individuoF4)
		ordenaPopulacao(populacao)
